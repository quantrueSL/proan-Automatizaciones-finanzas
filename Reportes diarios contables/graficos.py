...
def build_chart_descuentos(df, current_year, prior_year, out_path):
    """Comparativo por sociedad: barras de Ingresos vs Descuentos del año actual (mismo
    estilo que build_chart) + línea de % Descuentos/Ingresos en el eje secundario."""
    df = df.copy()
    df["orden"] = df["ingresos_actual"].abs()
    df = df.sort_values("orden", ascending=False)

    truncado = len(df) > MAX_SOCIEDADES_EN_GRAFICO
    df = df.head(MAX_SOCIEDADES_EN_GRAFICO)

    divisor, unidad = _elegir_unidad(df[["ingresos_actual", "descuentos_actual"]].abs().to_numpy().max())
    ...
    x = np.arange(len(df))
    width = 0.36
    ax1.bar(x - width / 2, df["ingresos_actual"] / divisor, width,
            color=COLORS["actual"], label=f"Ingresos {current_year} (Millones $)", zorder=2)
    ax1.bar(x + width / 2, df["descuentos_actual"] / divisor, width,
            color=COLORS["critical"], label=f"Descuentos {current_year} (Millones $)", zorder=2)
    ...
    # % Descuentos/Ingresos del año actual, topado a +/-PCT_CAP igual que build_chart.
    # Donde no hay base material (NaN) se traza como 0 para no cortar la línea; ese punto
    # se marca aparte (hueco + "N/A") para no confundirlo con un 0% real.
    pct_original = df["pct_actual"]
    pct_capped = pct_original.fillna(0.0).clip(-PCT_CAP, PCT_CAP)
    ax2 = ax1.twinx()
    ax2.plot(x, pct_capped * 100, color=COLORS["header_bg"], marker="o", markersize=4.5,
              linewidth=1.6, label=f"% Descuentos/Ingresos {current_year} (máx ±{PCT_CAP:.0%})", zorder=3)

    for i, pct in enumerate(pct_original):
        if pct != pct:  # NaN: base no material, marcado como hueco sobre la línea unida
            ax2.plot(x[i], pct_capped.iloc[i] * 100, marker="o", markersize=5.5,
                      markerfacecolor=COLORS["surface"], markeredgecolor=COLORS["header_bg"],
                      markeredgewidth=1.4, zorder=4)
            ax2.annotate("N/A", (x[i], pct_capped.iloc[i] * 100), textcoords="offset points",
                         xytext=(0, 8), ha="center", fontsize=6.5, color=COLORS["muted"],
                         fontweight="bold")
    ax2.set_ylabel("% Descuentos/Ingresos", fontsize=9, color=COLORS["header_bg"])
    ax2.tick_params(axis="y", colors=COLORS["header_bg"], labelsize=8, length=0)
    ax2.yaxis.set_major_formatter(mticker.FuncFormatter(lambda v, _: f"{v:.0f}%"))
    ax2.set_ylim(-PCT_CAP * 100 - 20, PCT_CAP * 100 + 20)
    for spine in ("top", "left"):
        ax2.spines[spine].set_visible(False)
    ax2.spines["right"].set_color(COLORS["header_bg"])

    # Marca los puntos que sí se recortaron (excedían el tope) para no ocultar que hay más.
    for i, pct in enumerate(df["pct_actual"]):
        if pct == pct and abs(pct) > PCT_CAP:
            y_marca = (PCT_CAP if pct > 0 else -PCT_CAP) * 100
            ax2.annotate(f"{pct:+.0%}", (x[i], y_marca), textcoords="offset points",
                         xytext=(0, 8 if pct > 0 else -12), ha="center", fontsize=6.5,
                         color=COLORS["header_bg"], fontweight="bold")

    ax1.set_title("Descuentos y Bonificaciones", fontsize=13, color=COLORS["text_primary"],
