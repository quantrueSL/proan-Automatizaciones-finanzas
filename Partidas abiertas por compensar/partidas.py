...
def _grafico_evolucion_png(
    ...
    _grid_evolucion), no hay sitio para mas. El grafico grande (mini=False: el importe
    neto total del consolidado y la serie del correo por sociedad) SI lleva eje de
    fechas y el valor de cada punto en formato abreviado (1.1M, 859.2k): en el correo no
    se puede pasar el raton por encima para verlo, asi que el dato va escrito.
    ...
    # distancia a cero. En el grafico grande el colchon es mayor: hace falta hueco para
    # las etiquetas de valor de cada punto, que en el mini no existen.
...
def _bloque_evolucion(titulo: str, fechas: list[Any], valores: list[Decimal]) -> str:
    ...
    png = _grafico_evolucion_png(fechas, valores, ancho=820, alto=220, mini=False)
    img = _img_grafico(png, 820, 220, f"Evolucion — {titulo}")
    return (
        f'<p class="titulo-seccion" style="font-size:13px;color:{AZUL};font-weight:700;'
...
def _grid_evolucion(
    ...
    for cod in sociedades:
        ...
        if not valores or all(v == 0 for v in valores):
            continue
        png = _grafico_evolucion_png(fechas, valores, ancho=190, alto=56, mini=True)
